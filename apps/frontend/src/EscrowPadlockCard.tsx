...
const STATUS_COPY: Record<EscrowStatus, { label: string; caption: string }> = {
    LOCK_REQUESTED: {
        label: "Requesting payment",
        caption: "Sending Request to Pay via MoMo Collections…",
    },
    LOCK_PENDING: {
        label: "Awaiting approval",
        caption: "Waiting for the buyer to approve on their phone…",
    },
    LOCKED: {
        label: "Funds locked in escrow",
        caption: "Held securely until delivery is confirmed.",
    },
    LOCK_FAILED: {
        label: "Payment request failed",
        caption: "The buyer declined or the request timed out.",
    },
    RELEASE_REQUESTED: {
        label: "Releasing funds",
        caption: "Sending Transfer via MoMo Disbursements…",
    },
    RELEASE_PENDING: {
        label: "Transfer in progress",
        caption: "Funds are moving to the supplier's MoMo wallet…",
    },
    RELEASED: { label: "Funds released", caption: "Supplier has been paid." },
    RELEASE_FAILED: {
        label: "Release failed",
        caption: "The transfer could not be completed.",
    },
};

function iconFor(status: EscrowStatus) {
    if (status === "LOCKED")
        return <Lock size={28} strokeWidth={1.75} className="text-white" />;
    if (status === "RELEASED")
        return <LockOpen size={28} strokeWidth={1.75} className="text-white" />;
...
function badgeColor(status: EscrowStatus) {
    if (status === "LOCKED" || status === "RELEASED")
        return "var(--fluent-success, #00875A)";
    if (status === "LOCK_FAILED" || status === "RELEASE_FAILED")
        return "var(--fluent-danger, #D32F2F)";
        ...
        <SectionCard className="p-4 overflow-hidden">
            ...
            <div className="flex items-center gap-3.5">
                ...
                <div className="flex-1 min-w-0">
                    ...
                    {reference && (
                        <p className="app-micro text-[#8E8E93] mt-1 font-fluent-mono">
                            Ref: {reference}
                        </p>
                    )}
                </div>
