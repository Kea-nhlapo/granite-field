  ...
  for (let pageNum = 1; pageNum <= pdf.numPages; pageNum++) {
    ...
    const flush = () => {
      if (currentLine.length > 0) {
        lines.push(currentLine.map((i) => i.text).join(" ").replace(/\s+/g, " ").trim());
      }
    };
...
const LINE_ITEM_PATTERN = new RegExp(`^(.{3,60}?)\\s+(\\d{1,5})\\s+${MONEY}(?:\\s+${MONEY})?$`);
const INVOICE_NUMBER_PATTERN = /\b(?:invoice|inv)[\s#:.-]*([A-Z0-9-]{4,20})/i;
const TOTAL_PATTERN = new RegExp(`\\btotal(?:\\s+due|\\s+amount)?\\s*[:\\s]*${MONEY}`, "i");
...
export async function parseInvoicePdf(file: File): Promise<ParsedInvoice> {
  ...
  const totalMatches = [...fullText.matchAll(new RegExp(TOTAL_PATTERN, "gi"))];
