...
import { MapPin, MessageCircle, Navigation2, Clock, Star, Store, Truck } from "lucide-react";
...
function MapBackdrop() {
  return (
    <div className="absolute inset-0 overflow-hidden" style={{ backgroundColor: "#E8EBEE" }}>
      <svg viewBox="0 0 375 700" className="w-full h-full" preserveAspectRatio="xMidYMid slice">
        <rect width="375" height="700" fill="#E8EBEE" />
        {/* soft green park block, echoes the reference's Prospect Park */}
        <rect x="20" y="90" width="130" height="150" rx="10" fill="#DCE6DC" />
        <rect x="230" y="380" width="120" height="160" rx="10" fill="#DCE6DC" />
        {/* city blocks */}
        {Array.from({ length: 26 }).map((_, i) => {
          ...
          const w = 40 + ((i * 17) % 30);
          const h = 30 + ((i * 23) % 40);
          return <rect key={i} x={x} y={y} width={w} height={h} rx="3" fill="#DADEE2" />;
        })}
        {/* streets */}
        {[70, 150, 230, 310].map((x) => (
          <line key={`v${x}`} x1={x} y1="0" x2={x} y2="700" stroke="#F5F6F7" strokeWidth="6" />
        ))}
        {[100, 220, 340, 460, 580].map((y) => (
          <line key={`h${y}`} x1="0" y1={y} x2="375" y2={y} stroke="#F5F6F7" strokeWidth="6" />
        ))}
        <line x1="0" y1="0" x2="375" y2="700" stroke="#F0F2F3" strokeWidth="14" opacity="0.6" />
      </svg>
    </div>
  );
}
        ...
        <div className="px-5 pt-1.5">
          ...
          <div className="flex items-center gap-3">
            <div
              ...
              style={{ backgroundColor: "#EBF3FC", color: "#003E85", border: "1px solid #C7E0F4" }}
            >
              T
            </div>
            <div className="flex-1 min-w-0">
              <p className="app-heading truncate">Thabo Distributors</p>
              <div className="flex items-center gap-1 app-caption text-[#595959]">
                <Star size={12} className="text-[#F57C00] fill-[#F57C00]" />
                <span>4.8</span>
                <span>•</span>
          ...
          >
            ...
            <div className="w-px h-8 shrink-0" style={{ backgroundColor: "var(--fluent-stroke-divider, #E5E7EB)" }} />
          ...
          <div className="flex items-start gap-2.5 mt-4">
            <div className="flex flex-col items-center pt-1 shrink-0">
              <Navigation2 size={13} className="text-[#00875A]" strokeWidth={2.2} />
              <span className="w-px h-5 my-0.5" style={{ backgroundColor: "#D1D5DB" }} />
              <Clock size={13} className="text-[#003E85]" strokeWidth={2.2} />
            </div>
            <div className="flex-1 min-w-0 space-y-2.5">
              <p className="app-caption text-[#1A1A1A] truncate">Your Store — Fetsani Rd, Soweto</p>
              <p className="app-caption text-[#1A1A1A] truncate">Thabo Distributors — 14 Falcon Ave</p>
            </div>
          </div>

          <div className="flex flex-col gap-2.5 mt-5">
            <PrimaryBtn label="Confirm & Start Trade" />
            <SecondaryBtn label="Message Supplier" icon={<MessageCircle size={16} strokeWidth={2} />} />
          </div>
        </div>
      </m.div>
    </div>
