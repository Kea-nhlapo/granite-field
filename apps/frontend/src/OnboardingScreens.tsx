...
const SLIDES: Slide[] = [
  ...
  {
    icon: <ShieldCheck size={96} strokeWidth={1.5} className="text-white" />,
    title: "Trade with",
...
function MascotBadge({ children, spin = false }: { children: ReactNode; spin?: boolean }) {
  const style = {
    width: 176,
    height: 176,
...
function SplashIntro() {
  ...
  return (
    <m.div
      ...
      style={{ background: "linear-gradient(160deg, var(--momo-blue, #003E85), var(--momo-navy, #002B49))" }}
      initial={{ opacity: 1 }}
    ...
    >
      ...
      >
        <ShieldCheck size={44} strokeWidth={1.75} className="text-white" />
      </m.div>
    </m.div>
  );
}
...
export function OnboardingScreen({ onDone }: { onDone: () => void }) {
  ...
  useEffect(() => {
    const timer = setTimeout(() => setShowSplash(false), reduceMotion ? 300 : 1300);
  ...
  return (
    <div className="h-full flex flex-col overflow-hidden relative" style={{ backgroundColor: "#FFFFFF" }}>
      ...
      <div className="flex-1 relative overflow-hidden">
        <AnimatePresence initial={false} custom={direction}>
          ...
          >
            ...
            <MascotBadge spin={index === 0}>{slide.icon}</MascotBadge>

            <div className="flex flex-col gap-3">
              <h1 className="text-2xl font-bold leading-tight" style={{ color: "var(--momo-navy, #002B49)" }}>
                {slide.title}{" "}
                <span style={{ color: "var(--momo-blue, #003E85)" }}>{slide.highlight}</span>
              </h1>
              <p className="text-sm leading-relaxed" style={{ color: "var(--fluent-text-secondary, #595959)" }}>
                {slide.body}
              </p>
            </div>
          </m.div>
        ...
        {SLIDES.map((_, i) => (
          <button
            ...
            style={{
              width: i === index ? 24 : 8,
              height: 8,
              backgroundColor: i === index ? "var(--momo-navy, #002B49)" : "var(--momo-blue-border, #C7E0F4)",
            }}
          />
        ))}
      </div>
      ...
      >
        <PrimaryBtn label={isLast ? "Get Started" : "Continue"} onClick={next} className="h-12 text-base" />
        <SecondaryBtn label="Sign In" onClick={onDone} className="h-12 text-base" />
        <p className="text-center text-xs pt-1" style={{ color: "var(--fluent-text-tertiary, #8E8E93)" }}>
          By continuing, you agree to TradeMesh's{" "}
          <span style={{ color: "var(--momo-blue, #003E85)", fontWeight: 600 }}>Terms of Service</span> and{" "}
    ...
    <div className="h-full flex flex-col overflow-y-auto fluent-scroll" style={{ backgroundColor: "#FFFFFF" }}>
      ...
      >
        ...
        <div
          ...
          style={{
            borderLeft: "3px solid var(--momo-blue-border, #C7E0F4)",
        ...
        <div className="mt-6 w-[56%] mx-auto">
          <PrimaryBtn label="Continue" onClick={onSignedIn} className="h-12 text-base" />
        </div>

        <div className="flex items-center gap-3 mt-4">
          <div className="h-px flex-1" style={{ backgroundColor: "var(--fluent-stroke-divider, #E5E7EB)" }} />
          <span className="text-xs" style={{ color: "var(--fluent-text-tertiary, #8E8E93)" }}>
            or
          </span>
          <div className="h-px flex-1" style={{ backgroundColor: "var(--fluent-stroke-divider, #E5E7EB)" }} />
        </div>
        ...
        >
          <img src={momoLogo} alt="" aria-hidden="true" className="h-7 w-7 object-contain" />
          Sign in with MoMo
        </button>
