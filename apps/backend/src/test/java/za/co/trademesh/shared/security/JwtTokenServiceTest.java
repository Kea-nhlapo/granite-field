...
class JwtTokenServiceTest {
    ...
    void accessTokenIsSignedShortLivedAndCarriesRoles() {
        ...
        JwtTokenService.AccessToken token =
                service.issue(userId, Set.of(AccountRole.SUPPLIER, AccountRole.BUSINESS_OWNER));

        Jwt decoded = decoder.decode(token.value());
        assertThat(decoded.getSubject()).isEqualTo(userId.toString());
        assertThat(decoded.getIssuer().toString()).isEqualTo("https://trademesh.test");
        assertThat(decoded.getClaimAsStringList("roles")).containsExactly("BUSINESS_OWNER", "SUPPLIER");
