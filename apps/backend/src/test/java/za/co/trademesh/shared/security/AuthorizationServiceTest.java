...
class AuthorizationServiceTest {
    ...
    void ordinaryBusinessUsersCannotReadInternalRiskOrInsurerData() {
        ...
        assertThatThrownBy(() -> authorization.requireInternalRiskAccess(owner))
                .isInstanceOf(AccessDeniedException.class);
        assertThatThrownBy(() -> authorization.requireInsurerAccess(owner)).isInstanceOf(AccessDeniedException.class);
    }

    @Test
    void sensitiveRolesRemainSeparate() {
        assertThatCode(() -> authorization.requireInternalRiskAccess(authentication(AccountRole.INTERNAL_RISK_ANALYST)))
                .doesNotThrowAnyException();
        assertThatCode(() -> authorization.requireInsurerAccess(authentication(AccountRole.INSURER)))
                .doesNotThrowAnyException();

        assertThatThrownBy(() -> authorization.requireInsurerAccess(authentication(AccountRole.INTERNAL_RISK_ANALYST)))
                .isInstanceOf(AccessDeniedException.class);
    }
    ...
    private static Authentication authentication(AccountRole role) {
        return UsernamePasswordAuthenticationToken.authenticated(
                USER_ID.toString(), "not-used", List.of(new SimpleGrantedAuthority("ROLE_" + role.name())));
    }
}
